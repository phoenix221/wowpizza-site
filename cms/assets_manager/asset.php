...
function stylesheets($params){
    ...
    $concat=false;
    $timeweb_time = 0;
    if(strpos($_SERVER['HTTP_HOST'], 'apf.su') !== false || strpos($_SERVER['HTTP_HOST'], 'foodcosmos.ru') !== false){
        $timeweb_time = 3600*3;
    }
    ...
    foreach ($params as $key=>$file){
        ...
        if(substr($file,-5)=='.scss'){
            ...
            if(!file_exists($_SERVER['DOCUMENT_ROOT'].$file.$suffix) || filemtime($_SERVER['DOCUMENT_ROOT'].$file )+$timeweb_time > filemtime($_SERVER['DOCUMENT_ROOT'].$file.$suffix) || $is_another_files){
                $reconcat=true;
                if($minify){
                    file_put_contents(
                        $_SERVER['DOCUMENT_ROOT'].$file.$suffix, //куда записываем
