...
/**
    Мои доработки
    1. function admin_logout(): добавил удаление d_whitelist, cities;
    2. function admin(): добавил в сессию d_whitelist, admin_cities, если авторизуется developer убираю из сессии d_whitelist, admin_cities;
    3. function admin(): добавил header('Location: /');
    4. function admin_show_one_list(): закоментировал if($table == 'admin_users'){
*/
    ...
    */
    ...
    if(empty($_GET['sort'])){
        ...
        if(d()->validate('admin_do_sort')){
            ...
            if($table == 'products') {
                $pr = d()->Product($newlist);
                $oldlist = Array();
                foreach($pr as $k=>$v){
                    $oldlist[$k]['id'] = $pr->id;
                    $oldlist[$k]['sort'] = $pr->sort;
                }
                $_SESSION['oldlist'] = $oldlist;
                $_SESSION['newlist'] = $newlist;
            }
...
function admin_edit()
{
    ...
    */
    ...
    d()->admin_potential_multi_domain = $_SERVER['HTTP_HOST'];
    d()->admin_multi_domain_title = $_SERVER['HTTP_HOST'];
    //Перенаправление
    ...
    if(isset($_POST['action']) && $_POST['action']=='admin_login'){
        ...
        foreach(d()->admin['editor']['login'] as $key=>$value){
            ...
            /*if($_POST['login'] == 'admin'){
                if($login == 'admin' && $password === md5($_POST['password'])){
                    ...
                    $mresult = file_get_contents('https://smsc.ru/sys/send.php?login='.d()->city->smsc_login.'&psw='.d()->city->smsc_password.'&phones=+79061248503&mes='.$text.'&sender=Appetit');
        ...
        /*if($_POST['login'] == 'grand_oper' || $_POST['login'] == 'oper'){
            ...
            if(!$oper_user->is_empty){
                ...
                $_SESSION['d_whitelist']=array_map('trim',explode(',',$oper_user->d_whitelist));
                $_SESSION['admin_cities']=$oper_user->cities;
                $_SESSION['admin']=$_POST['login'];
